...
export default function InvitationsList({ 
  ...
  const fetchInvitations = async () => {
    try {
      ...
      // Filter invitations by organization if activeOrgId is provided
      const filteredReceived = activeOrgId 
        ? received.filter(inv => inv.status === 'pending' && inv.organization_id === activeOrgId)
        : received.filter(inv => inv.status === 'pending');
        
      const filteredSent = activeOrgId 
        ? sent.filter(inv => inv.status === 'pending' && inv.organization_id === activeOrgId)
        : sent.filter(inv => inv.status === 'pending');
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden mb-8">
      ...
      {showReceived && receivedInvitations.length > 0 && (
        <div className="p-5 border-b border-gray-200 dark:border-gray-700">
