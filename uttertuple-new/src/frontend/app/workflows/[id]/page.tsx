...
import { Phone, PlayCircle, ArrowLeft, ChevronLeft, UserPlus } from 'lucide-react';
...
import AgentEditPanel from '@/components/workflows/AgentEditPanel';
import CreateAgentPanel from '@/components/workflows/CreateAgentPanel';
...
const CreateWorkflowPage: NextPage = () => {
  ...
  // Agent edit panel (right side) - for editing agent params directly on workflow page
  const [createAgentPanelOpen, setCreateAgentPanelOpen] = useState(false);
  const [agentEditPanel, setAgentEditPanel] = useState<{
    isOpen: boolean;
    nodeId: string | null;
    agentId: string | null;
    nodeLabel?: string;
  ...
  const onDragStart = (event: React.DragEvent<HTMLDivElement>, agentId: string) => {
    // Don't start drag when clicking Edit or other buttons
    ...
    // Compact custom drag preview - inline styles to avoid the default wide bar
    ...
    dragPreview.style.cssText = `
      ...
      pointer-events: none;
    `;
    dragPreview.textContent = agent.name;
    document.body.appendChild(dragPreview);
    // Use fixed offsets - compact pill is ~60px wide, ~24px tall
    event.dataTransfer.setDragImage(dragPreview, 60, 12);
  ...
  const handleContextMenuEdit = (id: string) => {
    ...
    if (contextMenu.type === 'node') {
      ...
      if (node) {
        // For agent nodes with agentId, open the right-side edit panel instead of dialog
        if (node.type === 'agent') {
          const agentData = node.data as AgentNodeData;
          if (agentData.agentId) {
            setAgentEditPanel({
  ...
  return (
    <MainLayout>
      ...
      <style jsx global>{`
        ...
        /* Crisp rendering for agents panel - prevents blur from transforms */
        .agents-panel-crisp {
      ...
      <div className={`flex h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
          ...
          <AnimatePresence mode="wait">
            {!agentsSidebarCollapsed && (
              <motion.div 
                ...
                className="h-full overflow-y-auto agents-panel-crisp"
              >
                <div className="p-4">
                  <motion.div 
                  ...
                  {/* Create Agent button */}
                  <button
                    onClick={() => setCreateAgentPanelOpen(true)}
                    className={`flex items-center justify-center gap-2 w-full py-2.5 px-3 mb-4 rounded-lg text-sm font-medium transition-all duration-200 ${
                      darkMode
                      ...
                      .map((agent, index) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setAgentEditPanel({ isOpen: true, nodeId: null, agentId: agent.id, nodeLabel: agent.name });
                            }}
                            className={`absolute top-0 right-0 z-10 flex items-center gap-1 px-2 py-1 rounded-bl rounded-tr-lg text-xs font-medium transition-colors duration-200 ${
                              darkMode 
                                ? 'text-gray-300 hover:text-white bg-gray-700/80 hover:bg-gray-600' 
                                : 'text-gray-600 hover:text-gray-800 bg-gray-100/90 hover:bg-gray-200'
                            }`}
                          ...
                          {/* Agent Header */}
                          <div className={`flex items-start mb-3 pt-6 ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                            {/* Agent icon/avatar */}
                            <motion.div 
        ...
        {/* Main workflow area + optional right agent edit panel */}
        ...
        <div className="flex-1 flex flex-col relative min-w-0">
          ...
          {/* Right-side panels: Create Agent or Edit Agent */}
          {createAgentPanelOpen ? (
            <CreateAgentPanel
              onClose={() => setCreateAgentPanelOpen(false)}
              onAgentCreated={(agent) => {
                const normalized: Agent = {
                  ...agent,
                  description: agent.description ?? '',
                };
                setRealAgents((prev) => [...prev, normalized]);
                setCreateAgentPanelOpen(false);
              }}
            />
          ) : agentEditPanel.isOpen && agentEditPanel.agentId ? (
            <AgentEditPanel
              agentId={agentEditPanel.agentId}
              nodeLabel={agentEditPanel.nodeLabel}
              onClose={() => setAgentEditPanel({ isOpen: false, nodeId: null, agentId: null, nodeLabel: undefined })}
              onAgentUpdated={(updatedAgent) => {
                ...
                setRealAgents((prev) =>
                  prev.map((a) => (a.id === updatedAgent.id ? normalized : a))
                );
                if (agentEditPanel.nodeId) {
                  setNodes((nds) =>
                    nds.map((n) => {
                      if (n.id !== agentEditPanel.nodeId || n.type !== 'agent') return n;
                      const data = n.data as AgentNodeData;
                      if (data.agentId !== updatedAgent.id) return n;
                      return {
                        ...n,
                        data: {
                          ...data,
                          llm_model: updatedAgent.llm_model,
                          tts_provider: updatedAgent.tts_config?.provider,
                          voice_id: updatedAgent.voice_id,
                          rag_count: updatedAgent.rag_config?.length ?? 0,
                          tools_count: updatedAgent.tools?.length ?? 0,
                          collection_fields_count: updatedAgent.collection_fields?.length ?? 0,
                        },
                      };
                    })
                  );
