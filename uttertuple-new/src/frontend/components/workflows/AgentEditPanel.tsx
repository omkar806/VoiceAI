...
// Voice options (matching agent edit page)
...
const AgentEditPanel: React.FC<AgentEditPanelProps> = ({
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (agentData.llm_provider_id) setSelectedLLMProvider(agentData.llm_provider_id);
        if (agentData.tts_provider_id) {
          setSelectedTTSProvider(agentData.tts_provider_id);
          const p = ttsData.find((x: TTSProvider) => x.id === agentData.tts_provider_id);
          if (p) setSelectedTTSProviderName(p.provider_name);
        }

        let voiceValue = agentData.voice_id || '';
        if (agentData.llm_model === 'openai-realtime' && agentData.llm_config?.voice) {
        ...
        setFormData({
          ...
          llm_config: agentData.llm_config ? { ...agentData.llm_config } : undefined,
          tts_provider_id: agentData.tts_provider_id,
          tts_config: agentData.tts_config,
        });

        if (agentData.rag_config?.length) {
          setRagConfigs(
            agentData.rag_config.map((r) => ({ dbId: r.id, collectionName: r.collection_name }))
          );
          setSelectedVectorDbs(agentData.rag_config.map((r) => r.id));
          agentData.rag_config.forEach((r) => fetchCollections(r.id));
        }

        if (agentData.tools) setTools(agentData.tools);
      } catch (err) {
  ...
  const handleAddToolSubmit = useCallback(async () => {
    try {
      const created = await agentService.createAgentTool(agentId, newTool);
  ...
  }, [agentId, newTool, handleCloseAddModal]);

  const handleRemoveTool = useCallback(async (index: number) => {
    const tool = tools[index];
    if (tool?.id) {
      try {
        await agentService.deleteAgentTool(agentId, tool.id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await agentService.updateAgent(agentId, updateData);
      setAgent(updated);
